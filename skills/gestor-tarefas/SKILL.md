...
- **"Quais destes têm tarefa pendente?"** — contar tarefas por **entidade vinculada, em lote**
  (vários pacientes/atendimentos/agendamentos de uma vez) → ferramenta de **contagem por vínculo**.
  É o que acende o **badge** no contexto (agenda/check-in/ficha): responde de uma vez, sem listar,
  quantas tarefas **abertas** cada item tem — e conta **uma só** quando a tarefa está ligada a mais
  de um deles (ex.: ao agendamento **e** ao paciente). Para a **lista** em si, use a busca.
